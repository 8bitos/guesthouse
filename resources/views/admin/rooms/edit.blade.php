...
<body class="bg-[#F8FAFC] text-gray-900 font-sans min-h-screen flex flex-col justify-between">
            ...
            <form method="POST" action="{{ route('admin.rooms.update', $room) }}" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-8">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="hidden" id="size" name="size" value="{{ old('size', $room->size ?? 25) }}">
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <select id="status" name="status" required
                                class="w-full bg-transparent border-b-2 border-gray-200 focus:border-amber-700 focus:outline-none py-2 text-gray-700 transition">
                            <option value="tersedia" {{ old('status', $room->status) === 'tersedia' ? 'selected' : '' }}>Available</option>
                            <option value="dipesan" {{ old('status', $room->status) === 'dipesan' ? 'selected' : '' }}>Booked</option>
                            <option value="perbaikan" {{ old('status', $room->status) === 'perbaikan' ? 'selected' : '' }}>Maintenance</option>
                        </select>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @if($room->addons && is_array($room->addons))
                @foreach($room->addons as $addon)
                    addAddon(
                        "{{ addslashes($addon['name'] ?? '') }}",
                        "{{ (int)($addon['price'] ?? 0) }}",
                        "{{ $addon['type'] ?? 'flat_fee' }}",
                        "{{ addslashes($addon['description'] ?? '') }}"
                    );
                @endforeach
            @else
